...
import { deleteFromCart, initialiseData } from '../../store/features/cartSlice';
...
const cart = () => {
    ...
    useEffect(() => {
        ...
        if (bottomSheetRef.current) {
            bottomSheetRef.current.scrollTo(-(viewHeight + 35));
          }
        //   console.log(cart);
        //   dispatch(initialiseData());
    }, [viewHeight, clickedProduct]);
    const handleRemoveFromCart = () => {
        const adId = clickedProduct.id
        dispatch(deleteFromCart(adId));
        setTimeout((() => {
            closeBottomSheet()
            openModal()
            setTitle('Success !')
            setMessage('Ad retrieve from cart successfully !')
        }), 100)
    }
    ...
    const deleteAction = (id) => {
        ...
        setClickedProduct(filteredList[0]);
                        ...
                        <View onLayout={handleLayout} className="flex justify-center w-full h-fit py-6 items-center flex-col mx-auto">
                            <View >
                                ...
                                <View className="flex mt-4 justify-between items-center space-x-3 flex-row w-full">
                                    <TouchableOpacity activeOpacity={0.8} onPress={closeBottomSheet} className="border flex-1 border-principal px-6 py-3 items-center flex flex-row space-x-2 justify-center rounded-full">
